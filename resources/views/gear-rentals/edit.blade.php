...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-4">Edit Gear Rental</h1>

    <form action="{{ route('gear-rentals.update', $gearRental->id) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="gear_id" id="gear_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
                @foreach ($gearStocks as $gearStock)
                    <option value="{{ $gearStock->id }}" {{ $gearStock->id == $gearRental->gear_id ? 'selected' : '' }}>
                        {{ $gearStock->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="user_id" id="user_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" required>
                @foreach ($users as $user)
                    <option value="{{ $user->id }}" {{ $user->id == $gearRental->user_id ? 'selected' : '' }}>
                        {{ $user->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="rental_date" id="rental_date" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" value="{{ $gearRental->rental_date }}" required>
            @error('rental_date')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="return_date" id="return_date" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50" value="{{ $gearRental->return_date }}">
            @error('return_date')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
